...
def build_sequence_features(guide, target):
    ...
    arr = np.array(seq_array_unit, dtype=np.float32).reshape(160, 4)
...
def normalize_target(value):
    normalized_value = (value - (-48)) / (-22 - (-48))
    return normalized_value

def normalize_guide_conse(value):
    normalized_value = (value - (-54)) / (3 - (-54))
    return normalized_value

def normalize_target_conse(value):
    normalized_value = (value - (-40)) / (-14 - (-40))
    return normalized_value
