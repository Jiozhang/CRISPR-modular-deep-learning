...
class CNNBranch1(nn.Module):
    ...
    def __init__(self, filters, kernel_size, dense_units):
        ...
        self.fc = nn.Linear(filters*39, dense_units)
    def forward(self, x):
        x = x.permute(0,2,1)  # (batch,4,39)
        x = F.relu(self.conv(x))
        x = self.pool(x)      # (batch,filters,39)
...
class CNNBranch2(nn.Module):
    """
    CNN branch for (158,3).
    """
    def __init__(self, filters, kernel_size, dense_units):
        super().__init__()
        self.conv = nn.Conv1d(3, filters, kernel_size, padding=(kernel_size-1)//2)
        self.pool = nn.MaxPool1d(2)
        self.fc = nn.Linear(filters*79, dense_units)
    def forward(self, x):
        x = x.permute(0,2,1)  # (batch,3,158)
        x = F.relu(self.conv(x))
        x = self.pool(x)      # (batch,filters,79)
        x = x.flatten(start_dim=1)
        x = F.relu(self.fc(x))
        return x
