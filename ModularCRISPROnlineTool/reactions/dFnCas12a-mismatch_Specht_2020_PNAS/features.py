...
def build_sequence_features(guide, target):
    ...
    arr = np.array(seq_array_unit, dtype=np.float32).reshape(79, 4)
    return torch.tensor(arr, dtype=torch.float32).unsqueeze(0)

# -----------------------------
# 2. Structure features: x2
# shape should be (1, 158, 3)
# -----------------------------
...
def build_structure_features(guide, target):
    ...
    arr = np.array(struct_array_unit, dtype=np.float32).reshape(158, 3)
    return torch.tensor(arr, dtype=torch.float32).unsqueeze(0)
...
def normalize_guide(value):
    normalized_value = (value - (-3)) / (1 - (-3))
    return normalized_value

def normalize_target(value):
    normalized_value = (value - (-25)) / (-12 - (-25))
    return normalized_value

def normalize_guide_conse(value):
    normalized_value = (value - (-14)) / (0 - (-14))
    return normalized_value

def normalize_target_conse(value):
    normalized_value = (value - (-23)) / (-7 - (-23))
    return normalized_value

def normalize_guide_conse_unpaired(value):
    normalized_value = (value - (-33)) / (-1 - (-33))
    return normalized_value

def normalize_target_conse_unpaired(value):
    normalized_value = (value - (-10)) / (0 - (-10))
    return normalized_value

def normalize_guide_overhang(value):
    normalized_value = (value - (-33)) / (3 - (-33))
    return normalized_value
...
def normalize_guide_paired(value):
    normalized_value = (value - (-14)) / (2 - (-14))
    return normalized_value

def normalize_target_paired(value):
    normalized_value = (value - (-23)) / (0 - (-23))
    return normalized_value

def normalize_seed(value):
    normalized_value = (value - (-8)) / (-6 - (-8))
    return normalized_value

def normalize_middle(value):
    normalized_value = (value - (-7)) / (-5 - (-7))
    return normalized_value

def normalize_distal(value):
    normalized_value = (value - (-9)) / (-7 - (-9))
    return normalized_value
...
def build_energy_features(guide, target):
    ...
    energy_array_unit = [normalize_guide(subopt_structures_guide[0].energy + 3.57)]
