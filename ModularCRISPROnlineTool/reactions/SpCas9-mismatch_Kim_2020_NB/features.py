...
def build_sequence_features(guide, target):
    ...
    arr = np.array(seq_array_unit, dtype=np.float32).reshape(172, 4)
    return torch.tensor(arr, dtype=torch.float32).unsqueeze(0)

# -----------------------------
# 2. Structure features: x2
# shape should be (1, 304, 3)
# -----------------------------
...
def build_structure_features(guide, target):
    ...
    arr = np.array(struct_array_unit, dtype=np.float32).reshape(304, 3)
    return torch.tensor(arr, dtype=torch.float32).unsqueeze(0)
...
def normalize_target(value):
    normalized_value = (value - (-49)) / (-28 - (-49))
    return normalized_value
...
def normalize_target_conse(value):
    normalized_value = (value - (-32)) / (-8 - (-32))
    return normalized_value
...
def normalize_guide_overhang(value):
    normalized_value = (value - (-28)) / (3 - (-28))
    return normalized_value
...
def normalize_target_paired(value):
    normalized_value = (value - (-32)) / (0 - (-32))
    return normalized_value
...
def normalize_middle(value):
    normalized_value = (value - (-12)) / (-4 - (-12))
    return normalized_value

def normalize_distal(value):
    normalized_value = (value - (-13)) / (-5 - (-13))
    return normalized_value
