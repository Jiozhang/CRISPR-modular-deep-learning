...
def normalize_guide(value):
    normalized_value = (value - (-21)) / (1 - (-21))
    return normalized_value

def normalize_target(value):
    normalized_value = (value - (-56)) / (-33 - (-56))
    return normalized_value

def normalize_guide_conse(value):
    normalized_value = (value - (-53)) / (3 - (-53))
    return normalized_value

def normalize_target_conse(value):
    normalized_value = (value - (-38)) / (-16 - (-38))
    return normalized_value

def normalize_guide_conse_unpaired(value):
    normalized_value = (value - (-45)) / (3 - (-45))
    return normalized_value
...
def build_energy_features(guide, target):
    ...
    energy_array_unit = [normalize_guide(subopt_structures_guide[0].energy + 27.32)]
