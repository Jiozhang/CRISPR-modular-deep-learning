...
TEST_GUIDE_LEN = 23
TEST_TARGET_LEN = 23
scaffold = "AACCCCUACCAACUGGUCGGGGUUUGAAAC"
...
# shape should be (1, 76, 4)
# -----------------------------
def build_sequence_features(guide, target):
    guide = scaffold + guide
    seq_array_unit = encode_sequence(guide) + encode_sequence(target) 
    arr = np.array(seq_array_unit, dtype=np.float32).reshape(76, 4)
    return torch.tensor(arr, dtype=torch.float32).unsqueeze(0)

# -----------------------------
# 2. Structure features: x2
# shape should be (1, 152, 3)
# -----------------------------
...
def build_structure_features(guide, target):
    ...
    subopt_structures_target_bh = subopt(strands=target, energy_gap=0.1, model=my_model_RNA)
    ...
    for j in range (0, len(target)):
        ...
        try:
            if str(subopt_structures_target_bh[0].structure)[j] == '.':
                to_be_added = ([1, 0, 0])
            elif str(subopt_structures_target_bh[0].structure)[j] == '(':
                to_be_added = ([0, 1, 0])
            elif str(subopt_structures_target_bh[0].structure)[j] == ')':
                to_be_added = ([0, 0, 1])
            else:
    ...
    arr = np.array(struct_array_unit, dtype=np.float32).reshape(152, 3)
    return torch.tensor(arr, dtype=torch.float32).unsqueeze(0)
...
def build_energy_features(guide, target):
    ...
    subopt_structures_target_bh = subopt(strands=target, energy_gap=0.1, model=my_model_RNA)
    subopt_structures_target = subopt(strands=[guide, target], energy_gap=0.1, model=my_model_RNA)
    ...
    if str(subopt_structures_target_bh[0].structure)== '.' * len_spacer:
        ensemble_energy_max_paired_target_bh = 0
    else:
        max_start_index_target_bh, max_length_target_bh = find_max_base_pairs(str(subopt_structures_target_bh[0].structure))
        max_paired_seq_target_bh = target[max_start_index_target_bh:max_start_index_target_bh+max_length_target_bh]
    ...
    if is_all_parens(str(subopt_structures_target_bh[0].structure)) == True:
        ensemble_energy_max_unpaired_target_bh = 0
    else:
        max_start_index_target_bh, max_length_target_bh = find_max_unpaired(str(subopt_structures_target_bh[0].structure))
        max_unpaired_seq_target_bh = target[max_start_index_target_bh:max_start_index_target_bh+max_length_target_bh]
    ...
    if str(subopt_structures_target_bh[0].structure)[0] != '.':
        ensemble_energy_5_overhang_target_bh = 0
    else:
        max_start_index_target_bh, max_length_target_bh = detect_5_overhang(str(subopt_structures_target_bh[0].structure))
        max_5_overhang_seq_target_bh = target[max_start_index_target_bh:max_start_index_target_bh+max_length_target_bh]
    ...
    if str(subopt_structures_target_bh[0].structure)[len_spacer-1] != '.':
        ensemble_energy_3_overhang_target_bh = 0
    else:
        max_start_index_target_bh, max_length_target_bh = detect_3_overhang(str(subopt_structures_target_bh[0].structure))
        max_3_overhang_seq_target_bh = target[max_start_index_target_bh:max_start_index_target_bh+max_length_target_bh]
    ...
    if str(subopt_structures_target_bh[0].structure)[0] == '.':
        ...
        max_start_index_target_bh, max_length_target_bh = detect_5_paired(str(subopt_structures_target_bh[0].structure))
        max_5_paired_seq_target_bh = target[max_start_index_target_bh:max_start_index_target_bh+max_length_target_bh]
    ...
    if str(subopt_structures_target_bh[0].structure)[len_spacer-1] == '.':
        ...
        max_start_index_target_bh, max_length_target_bh = detect_3_paired(str(subopt_structures_target_bh[0].structure))
        max_3_paired_seq_target_bh = target[max_start_index_target_bh:max_start_index_target_bh+max_length_target_bh]
    ...
    energy_array_unit = [normalize_guide(subopt_structures_guide[0].energy + 11.88)]
    ...
    energy_array_unit.append(normalize_target_bh(subopt_structures_target_bh[0].energy))
    energy_array_unit.append(normalize_target_bh_conse(ensemble_energy_max_paired_target_bh))
    energy_array_unit.append(normalize_target_bh_conse_unpaired(ensemble_energy_max_unpaired_target_bh))
    energy_array_unit.append(normalize_target_bh_overhang(ensemble_energy_5_overhang_target_bh))
    energy_array_unit.append(normalize_target_bh_overhang(ensemble_energy_3_overhang_target_bh))
    energy_array_unit.append(normalize_target_bh_paired(ensemble_energy_5_paired_target_bh))
    energy_array_unit.append(normalize_target_bh_paired(ensemble_energy_3_paired_target_bh))
...
def build_target_bh_graph(target):
    ...
    node_features = encode_sequence(target)
    edge_index_list = matrix_to_edge_index(prob_matrix)
    edge_features = generate_edge_features(edge_index_list, node_features, prob_matrix)
