...
class CNNBranch1(nn.Module):
    ...
    def __init__(self, filters, kernel_size, dense_units):
        ...
        self.fc = nn.Linear(filters*38, dense_units)
    def forward(self, x):
        x = x.permute(0,2,1)  # (batch,4,38)
        x = F.relu(self.conv(x))
        x = self.pool(x)      # (batch,filters,38)
...
class CNNBranch2(nn.Module):
    """
    CNN branch for (152,3).
    """
    def __init__(self, filters, kernel_size, dense_units):
        super().__init__()
        self.conv = nn.Conv1d(3, filters, kernel_size, padding=(kernel_size-1)//2)
        self.pool = nn.MaxPool1d(2)
        self.fc = nn.Linear(filters*76, dense_units)
    def forward(self, x):
        x = x.permute(0,2,1)  # (batch,3,76)
        x = F.relu(self.conv(x))
        x = self.pool(x)      # (batch,filters,76)
        x = x.flatten(start_dim=1)
        x = F.relu(self.fc(x))
        return x
    
class MLPBranch24(nn.Module):
    """
    A single hidden layer MLP for 24D => output dimension mlp_dim.
    """
...
class GNNBranch(nn.Module):
    def __init__(self, hidden_dim=32, edge_attr_dim=2):
        ...
        self.mlp_merge = nn.Sequential(
            nn.Linear(hidden_dim * 3 * 2, hidden_dim),  # 4 graphs, mean+max
            nn.ReLU(),
    ...
    def forward(self, gRNA_data, target_bh_data, duplex_data):
...
class CNN_GNN_MLP_Fusion(nn.Module):
    ...
    def __init__(self,
        ...
        self.mlp_branch = MLPBranch24(input_dim=24, hidden_dim=mlp_hidden_dim)
    ...
    def forward(self, gRNA_data, target_bh_data, duplex_data, x1, x2, x3):
        feat_cnn1 = self.cnn_branch1(x1)                   # (batch, dense_units1)
        feat_cnn2 = self.cnn_branch2(x2)                   # (batch, dense_units2)
        feat_gnn = self.gnn_branch(gRNA_data, target_bh_data, duplex_data)  # (batch, gnn_hidden_dim)
