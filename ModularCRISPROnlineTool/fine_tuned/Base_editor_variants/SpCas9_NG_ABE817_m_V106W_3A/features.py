...
TEST_GUIDE_LEN = 19
TEST_TARGET_LEN = 19
scaffold = "GUUUCAGAGCUAUGCUGGAAACAGCAUAGCAAGUUGAAAUAAGGCUAGUCCGUUAUCAACUUGAAAAAGUGGCACCGAGUCGGUGCUUUUUU"
...
def build_structure_features(guide, target):
    ...
    spacer_struct_bh = PAM_order(str(subopt_structures_guide_bh[0].structure)[0:TEST_GUIDE_LEN])
    for j in range (0, 30):
    ...
    spacer_struct_ah = PAM_order(hybrid_ah_1[0:TEST_GUIDE_LEN])
...
def normalize_guide(value):
    normalized_value = (value - (-12.62)) / (0 - (-12.62))
    return normalized_value

def normalize_target(value):
    normalized_value = (value - (-55.83)) / (-27.32 - (-55.83))
    return normalized_value

def normalize_ssDNA_target_bh(value):
    normalized_value = (value - (-10.06)) / (0 - (-10.06))
    return normalized_value

def normalize_guide_conse(value):
    normalized_value = (value - (-48.75)) / (2.61 - (-48.75))
    return normalized_value

def normalize_target_conse(value):
    normalized_value = (value - (-38.37)) / (0 - (-38.37))
    return normalized_value
...
def normalize_ssDNA_target_bh_conse_unpaired(value):
    normalized_value = (value - (-31.80)) / (-0.32 - (-31.80))
    return normalized_value
...
def normalize_ssDNA_target_bh_overhang(value):
    normalized_value = (value - (-31.80)) / (0 - (-31.80))
    return normalized_value
...
def normalize_seed(value):
    normalized_value = (value - (-10.99)) / (-3.63 - (-10.99))
    return normalized_value
...
def normalize_distal(value):
    normalized_value = (value - (-10.92)) / (-3.75 - (-10.92))
    return normalized_value
...
def build_energy_features(guide, target):
    ...
    energy_array_unit = [normalize_guide(subopt_structures_guide[0].energy + 27.31)]
...
def collapse_to_first_k_plus_1_sum(prob_matrix: np.ndarray, first_k: int = TEST_GUIDE_LEN) -> np.ndarray:
    """
