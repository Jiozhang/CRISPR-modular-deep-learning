...
TEST_GUIDE_LEN = 21
TEST_TARGET_LEN = 21
scaffold = "GUUUAAGUACUCUGGAAACAGAAUCUACUUAAACAAGGCAAAAUGCCGUGUUUAUCUCGUCAACUUGUUGGCGAGA"
...
def normalize_target(value):
    normalized_value = (value - (-55.29)) / (-22.45 - (-55.29))
    return normalized_value
...
def normalize_guide_conse(value):
    normalized_value = (value - (-54.36)) / (2.61 - (-54.36))
    return normalized_value

def normalize_target_conse(value):
    normalized_value = (value - (-42.59)) / (0 - (-42.59))
    return normalized_value
...
def normalize_ssDNA_target_bh_conse_unpaired(value):
    normalized_value = (value - (-35.29)) / (-0.32 - (-35.29))
    return normalized_value
...
def normalize_ssDNA_target_bh_overhang(value):
    normalized_value = (value - (-35.29)) / (0 - (-35.29))
    return normalized_value
...
def normalize_middle(value):
    normalized_value = (value - (-15.21)) / (-4.90 - (-15.21))
    return normalized_value

def normalize_distal(value):
    normalized_value = (value - (-13.07)) / (-4.54 - (-13.07))
    return normalized_value
...
def build_energy_features(guide, target):
    ...
    energy_array_unit = [normalize_guide(subopt_structures_guide[0].energy + 22.45)]
