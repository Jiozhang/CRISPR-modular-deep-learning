...
def build_structure_features(guide, target, params):
    ...
    guide = params.scaffold_seq + guide
    ...
    subopt_structures_target_ssRNA = subopt(strands=target, energy_gap=0.1, model=my_model_RNA)
    subopt_structures_hybrid_ah = subopt(strands=[guide, target], energy_gap=0.1, model=my_model_RNA)

    # Guide structure before hybridization
    spacer_struct_bh = str(subopt_structures_guide_bh[0].structure)[len(params.scaffold_seq):len(params.scaffold_seq)+params.spacer_len]
    for j in range (0, 30):
    ...
    # ssRNA target structure before hybridization 
    ...
    spacer_struct_ah = hybrid_ah_1[len(params.scaffold_seq):len(params.scaffold_seq)+params.spacer_len]
...
def normalize_ssRNA_target_bh(value, params):
...
def normalize_ssRNA_target_bh_conse(value, params):
...
def normalize_ssRNA_target_bh_conse_unpaired(value, params):
...
def normalize_ssRNA_target_bh_overhang(value, params):
...
def normalize_ssRNA_target_bh_paired(value, params):
...
def build_energy_features(guide, target, params):
    ...
    subopt_structures_target = subopt(strands=[guide, target], energy_gap=0.01, model=my_model_RNA)
    subopt_structures_ssRNA_target_bh = subopt(strands=target, energy_gap=0.01, model=my_model_RNA)
    
    len_scaffold = len(params.scaffold_seq)
    ...
    if str(subopt_structures_target[0].structure)[len_scaffold:len_scaffold+len_spacer] == '.' * len_spacer:
        ...
        partition_function_max_paired_target = pfunc(strands=[max_paired_seq_target, RNA_reverse_complement(max_paired_seq_target)], model=my_model_RNA)
        ensemble_energy_max_paired_target = partition_function_max_paired_target[1]

    if str(subopt_structures_ssRNA_target_bh[0].structure)[0:len_spacer] == '.' * len_spacer:
        ensemble_energy_max_paired_ssRNA_target_bh = 0
    else:
        max_start_index_ssRNA_target_bh, max_length_ssRNA_target_bh = find_max_base_pairs(str(subopt_structures_ssRNA_target_bh[0].structure)[0:len_spacer])
        max_paired_seq_ssRNA_target_bh = guide[max_start_index_ssRNA_target_bh:max_start_index_ssRNA_target_bh+max_length_ssRNA_target_bh]
        partition_function_max_paired_ssRNA_target_bh = pfunc(strands=[max_paired_seq_ssRNA_target_bh, RNA_reverse_complement(max_paired_seq_ssRNA_target_bh)], model=my_model_RNA)
        ensemble_energy_max_paired_ssRNA_target_bh = partition_function_max_paired_ssRNA_target_bh[1]
    ...
    if is_all_parens(str(subopt_structures_target[0].structure)[len_scaffold:len_scaffold+len_spacer]) == True:
        ...
        partition_function_max_unpaired_target = pfunc(strands=[max_unpaired_seq_target, RNA_reverse_complement(max_unpaired_seq_target)], model=my_model_RNA)
        ensemble_energy_max_unpaired_target = partition_function_max_unpaired_target[1]

    if is_all_parens(str(subopt_structures_ssRNA_target_bh[0].structure)[0:len_spacer]) == True:
        ensemble_energy_max_unpaired_ssRNA_target_bh = 0
    else:
        max_start_index_ssRNA_target_bh, max_length_ssRNA_target_bh = find_max_unpaired(str(subopt_structures_ssRNA_target_bh[0].structure)[0:len_spacer])
        max_unpaired_seq_ssRNA_target_bh = guide[max_start_index_ssRNA_target_bh:max_start_index_ssRNA_target_bh+max_length_ssRNA_target_bh]
        partition_function_max_unpaired_ssRNA_target_bh = pfunc(strands=[max_unpaired_seq_ssRNA_target_bh, RNA_reverse_complement(max_unpaired_seq_ssRNA_target_bh)], model=my_model_RNA)
        ensemble_energy_max_unpaired_ssRNA_target_bh = partition_function_max_unpaired_ssRNA_target_bh[1]
    ...
    if str(subopt_structures_target[0].structure)[len_scaffold+len_spacer-1] != '.':
        ...
        partition_function_PAM_distal_overhang_target = pfunc(strands=[max_PAM_distal_overhang_seq_target, RNA_reverse_complement(max_PAM_distal_overhang_seq_target)], model=my_model_RNA)
        ensemble_energy_PAM_distal_overhang_target = partition_function_PAM_distal_overhang_target[1]

    if str(subopt_structures_ssRNA_target_bh[0].structure)[0] != '.':
        ensemble_energy_PAM_distal_overhang_ssRNA_target_bh = 0
    else:
        max_start_index_ssRNA_target_bh, max_length_ssRNA_target_bh = detect_5_overhang(str(subopt_structures_ssRNA_target_bh[0].structure)[0:len_spacer])
        max_PAM_distal_overhang_seq_ssRNA_target_bh = guide[max_start_index_ssRNA_target_bh:max_start_index_ssRNA_target_bh+max_length_ssRNA_target_bh]
        partition_function_PAM_distal_overhang_ssRNA_target_bh = pfunc(strands=[max_PAM_distal_overhang_seq_ssRNA_target_bh, RNA_reverse_complement(max_PAM_distal_overhang_seq_ssRNA_target_bh)], model=my_model_RNA)
        ensemble_energy_PAM_distal_overhang_ssRNA_target_bh = partition_function_PAM_distal_overhang_ssRNA_target_bh[1]
    ...
    if str(subopt_structures_target[0].structure)[len_scaffold] != '.':
        ...
        partition_function_PAM_proximal_overhang_target = pfunc(strands=[max_PAM_proximal_overhang_seq_target, RNA_reverse_complement(max_PAM_proximal_overhang_seq_target)], model=my_model_RNA)
        ensemble_energy_PAM_proximal_overhang_target = partition_function_PAM_proximal_overhang_target[1]

    if str(subopt_structures_ssRNA_target_bh[0].structure)[len_spacer-1] != '.':
        ensemble_energy_PAM_proximal_overhang_ssRNA_target_bh = 0
    else:
        max_start_index_ssRNA_target_bh, max_length_ssRNA_target_bh = detect_3_overhang(str(subopt_structures_ssRNA_target_bh[0].structure)[0:len_spacer])
        max_PAM_proximal_overhang_seq_ssRNA_target_bh = guide[max_start_index_ssRNA_target_bh:max_start_index_ssRNA_target_bh+max_length_ssRNA_target_bh]
        partition_function_PAM_proximal_overhang_ssRNA_target_bh = pfunc(strands=[max_PAM_proximal_overhang_seq_ssRNA_target_bh, RNA_reverse_complement(max_PAM_proximal_overhang_seq_ssRNA_target_bh)], model=my_model_RNA)
        ensemble_energy_PAM_proximal_overhang_ssRNA_target_bh = partition_function_PAM_proximal_overhang_ssRNA_target_bh[1]
    ...
    if str(subopt_structures_target[0].structure)[len_scaffold+len_spacer-1] == '.':
        ...
        partition_function_PAM_distal_paired_target = pfunc(strands=[max_PAM_distal_paired_seq_target, RNA_reverse_complement(max_PAM_distal_paired_seq_target)], model=my_model_RNA)
        ensemble_energy_PAM_distal_paired_target = partition_function_PAM_distal_paired_target[1]

    if str(subopt_structures_ssRNA_target_bh[0].structure)[0] == '.':
        ensemble_energy_PAM_distal_paired_ssRNA_target_bh = 0
    else:
        max_start_index_ssRNA_target_bh, max_length_ssRNA_target_bh = detect_5_paired(str(subopt_structures_ssRNA_target_bh[0].structure)[0:len_spacer])
        max_PAM_distal_paired_seq_ssRNA_target_bh = guide[max_start_index_ssRNA_target_bh:max_start_index_ssRNA_target_bh+max_length_ssRNA_target_bh]
        partition_function_PAM_distal_paired_ssRNA_target_bh = pfunc(strands=[max_PAM_distal_paired_seq_ssRNA_target_bh, RNA_reverse_complement(max_PAM_distal_paired_seq_ssRNA_target_bh)], model=my_model_RNA)
        ensemble_energy_PAM_distal_paired_ssRNA_target_bh = partition_function_PAM_distal_paired_ssRNA_target_bh[1]
    ...
    if str(subopt_structures_target[0].structure)[len_scaffold] == '.':
        ...
        partition_function_PAM_proximal_paired_target = pfunc(strands=[max_PAM_proximal_paired_seq_target, RNA_reverse_complement(max_PAM_proximal_paired_seq_target)], model=my_model_RNA)
        ensemble_energy_PAM_proximal_paired_target = partition_function_PAM_proximal_paired_target[1]

    if str(subopt_structures_ssRNA_target_bh[0].structure)[len_spacer-1] == '.':
        ensemble_energy_PAM_proximal_paired_ssRNA_target_bh = 0
    else:
        max_start_index_ssRNA_target_bh, max_length_ssRNA_target_bh = detect_3_paired(str(subopt_structures_ssRNA_target_bh[0].structure)[0:len_spacer])
        max_PAM_proximal_paired_seq_ssRNA_target_bh = guide[max_start_index_ssRNA_target_bh:max_start_index_ssRNA_target_bh+max_length_ssRNA_target_bh]
        partition_function_PAM_proximal_paired_ssRNA_target_bh = pfunc(strands=[max_PAM_proximal_paired_seq_ssRNA_target_bh, RNA_reverse_complement(max_PAM_proximal_paired_seq_ssRNA_target_bh)], model=my_model_RNA)
        ensemble_energy_PAM_proximal_paired_ssRNA_target_bh = partition_function_PAM_proximal_paired_ssRNA_target_bh[1]
    
    # Dtermine seed_region, middle_region, and distal_region based on spacer length
    if len_spacer % 3 == 0 or len_spacer % 3 == 2:
        seed_region = target[len_spacer-len_spacer//3:len_spacer]
        middle_region = target[len_spacer-2*len_spacer//3:len_spacer-len_spacer//3]
        distal_region = target[0:len_spacer-2*len_spacer//3]
    elif len_spacer % 3 == 1:
        seed_region = target[len_spacer-len_spacer//3:len_spacer]
        middle_region = target[len_spacer-2*len_spacer//3-1:len_spacer-len_spacer//3]
        distal_region = target[0:len_spacer-2*len_spacer//3-1]
    
    # Calculate/compare target seed region free energy 
    subopt_structures_seed = subopt(strands=[seed_region, RNA_reverse_complement(seed_region)], energy_gap=6, model=my_model_RNA)
    seed_energy = subopt_structures_seed[0].energy

    # Calculate/compare target middle region free energy 
    subopt_structures_middle = subopt(strands=[middle_region, RNA_reverse_complement(middle_region)], energy_gap=6, model=my_model_RNA)
    middle_energy = subopt_structures_middle[0].energy

    # Calculate/compare target distal region free energy 
    subopt_structures_distal = subopt(strands=[distal_region, RNA_reverse_complement(distal_region)], energy_gap=6, model=my_model_RNA)
    ...
    energy_array_unit.append(normalize_ssRNA_target_bh(subopt_structures_ssRNA_target_bh[0].energy, params))
    energy_array_unit.append(normalize_ssRNA_target_bh_conse(ensemble_energy_max_paired_ssRNA_target_bh, params))
    energy_array_unit.append(normalize_ssRNA_target_bh_conse_unpaired(ensemble_energy_max_unpaired_ssRNA_target_bh, params))
    energy_array_unit.append(normalize_ssRNA_target_bh_overhang(ensemble_energy_PAM_proximal_overhang_ssRNA_target_bh, params))
    energy_array_unit.append(normalize_ssRNA_target_bh_overhang(ensemble_energy_PAM_distal_overhang_ssRNA_target_bh, params))
    energy_array_unit.append(normalize_ssRNA_target_bh_paired(ensemble_energy_PAM_proximal_paired_ssRNA_target_bh, params))
    energy_array_unit.append(normalize_ssRNA_target_bh_paired(ensemble_energy_PAM_distal_paired_ssRNA_target_bh, params))
...
def collapse_to_last_k_plus_1_sum(prob_matrix: np.ndarray, last_k: int) -> np.ndarray:
    """
...
def build_target_bh_graph(target):
    ...
    target BH graph:
      ...
      - zero node features
    """
    prob_matrix = pairs(strands=target, model=my_model_RNA).to_array()
...
def build_duplex_graph(guide, target, params):
    ...
    """
    k = params.spacer_len
    prob_matrix = pairs(strands=[guide, target], model=my_model_RNA).to_array()
